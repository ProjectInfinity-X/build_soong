...
func AutogeneratedRroModuleName(ctx android.EarlyModuleContext, moduleName, partition string) string {
	...
	product := strings.Replace(ctx.Config().Getenv("TARGET_PRODUCT"), "lineage_", "infinity_", 1)
	overrideName := strings.Replace(strings.Replace(strings.Replace(moduleName, "lineageos", "infinity", -1), "Lineage", "ProjectInfinityX", -1), "lineage", "infinity", -1)
